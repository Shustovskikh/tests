...
def course_order(courses, mentors, durations):
    courses_list = []
    for course, mentor, duration in zip(courses, mentors, durations):
        course_dict = {"title":course, "mentors":mentor, "duration":duration}
        courses_list.append(course_dict)

    indexes = list(range(len(courses_list)))

    indexes_d = sorted(indexes, key=lambda index: courses_list[index]["duration"])
    indexes_m = sorted(indexes, key=lambda index: len(courses_list[index]["mentors"]))

    return indexes_d == indexes_m, indexes_d, indexes_m

...
for id, dicted in enumerate(courses_list):
	key = dicted["duration"] # Получите значение из ключа duration
...
for id, course in enumerate(courses_list):
    ...
    if key not in durations_dict:
        durations_dict[key] = l
    elif course["duration"] == key:
        durations_dict.setdefault(key, [])
        durations_dict[key].append(id)
    else:
        durations_dict[key] = l.append(id)
